...
PFASBaleenCompSum <- Baleen_PFAS_data_comb %>% 
  ...
  group_by(ID_code, ID_code_corr, Sample_seq, Plate_num) %>% 
...
PFAS_BCS_woBwMn <-  ggplot(filter(PFASBaleenCompSum, 
                                  ...
                                  !ID_code_corr %in% c("COA200804Ba", "COA200808Ba")),
  ...
  geom_point(size = 3.5) + 
  labs(x = "", 
       y = bquote(paste("\u03A3"[10], "PFAS (ng/g d.w.)")),
       #color = "Species",
       shape = "Individual") +
  ...
  facet_wrap(.~ID_code_corr, scales = "free") +
...
PFAS_BCS_MnOnly <- ggplot(filter(PFASBaleenCompSum, 
  ...
  labs(x = "Baleen sample sequence", 
       y = bquote(paste("\u03A3"[10], "PFAS (ng/g d.w.)"))) +
  facet_wrap(.~ID_code_corr, scales = "free", ncol = 3, nrow = 2) +
  scale_colour_manual(values = pal,
                      guide = guide_legend(label.theme = element_text(face = "italic", size = 10))) +
...
ggsave("PFAS_BCS_MnOnly_v2.1.png", PFAS_BCS_MnOnly, width = 14, height = 6)
...
PFAS_NARW_only <-  ggplot(filter(PFASBaleenCompSum, 
  ...
  labs(y = bquote(paste("\u03A3"[10], "PFAS (ng/g d.w.)"))) +
...
ggsave("PFAS_NARW_only_v2.1.png", PFAS_NARW_only, width = 9, height = 5.5)
...
PFAS_Vector_baleen_p1 <- ggplot(filter(PFASBaleenCompSum, 
                                 SciName =="Megaptera novaeangliae",
                                 ID_code == "IFAW19-287Mn",
                                 Plate_num == "1"),
                          aes(x = -Sample_seq, y = Total_PFAS, color = abbr_binom(SciName),
...
PFAS_Vector_baleen_p1

ggsave("PFAS_Vector_baleen_p1_v2.png", PFAS_Vector_baleen_p1, width = 9, height = 5.5)





PFOS_Vector_baleen <- ggplot(filter(Baleen_PFAS_data_comb,
                                          ID_code == "IFAW19-287Mn",
                                          Plate_num == 1,
                                          Compound %in% c("PFOS")),
                                aes()) + 
